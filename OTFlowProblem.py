...
def stepRK4(odefun, z, Phi, alph, t0, t1):
    ...
    :param alph:   list, the 3 alpha values for regularization
...
def odefun(x, t, net, alph=[1.0,1.0,1.0]):
    """
    neural ODE combining the characteristics and log-determinant (see Eq. (2)), and
    the straight-line regularizer (see Eq. (7)).

    d_t  [x ; l ; r] = odefun( [x ; l ; r] , t )

    x - particle position
    l - log determinant (got from trace)
    r - accumulates violation of the straight-line regularization term
    """
    nex, d_extra = x.shape
    d = d_extra - 3
    ...
    jac, trace = net.trace_jac(z)

    dx = -net(z)
    dl = -trace.unsqueeze(1)
    dr = torch.norm(jac[:,-1] + torch.mm(jac[:,0:-1], net(z))) 
    
    return torch.cat( (dx,dl,dr) , 1  )
